"""
Scraper de Properati Ecuador (properati.com.ec) — Radar de Valorización Urbana.

Extrae de las páginas de resultados de venta: título, precio (USD), área,
habitaciones, ubicación y URL del aviso.
...
Uso:
    python scripts/scrapers/scrape_properati.py --city quito --pages 3
    python scripts/scrapers/scrape_properati.py --city guayaquil --pages 5 --output data/raw/scraped
    python scripts/scrapers/scrape_properati.py --city cuenca --pages 2
...
# Permitir ejecución directa: python scripts/scrapers/scrape_properati.py
...
logger = setup_logging(name="scrape_properati")

SOURCE_NAME = "properati"

# URLs de búsqueda verificadas (agosto 2026):
#   https://www.properati.com.ec/s/quito/venta
#   https://www.properati.com.ec/s/guayaquil/venta
#   https://www.properati.com.ec/s/cuenca/venta
CITIES: Dict[str, str] = {
    "quito": "https://www.properati.com.ec/s/quito/venta",
    "guayaquil": "https://www.properati.com.ec/s/guayaquil/venta",
    "cuenca": "https://www.properati.com.ec/s/cuenca/venta",
}

# Selectores de tarjetas (se usa el que más resultados devuelva).
...
# Un ancla "vacía" (#, javascript:) no es un enlace de aviso válido.
_SKIP_HREFS = ("#", "", "javascript:", "javascript:void(0)")


def _is_valid_card(card: Any, base_url: str) -> bool:
    """Una tarjeta válida debe contener un enlace de aviso y algo de precio."""
    for anchor in card.find_all("a", href=True):
        href = str(anchor["href"])
        if href in _SKIP_HREFS:
            continue
        resolved = urljoin(base_url, href)
        if "properati.com.ec" in resolved or resolved.startswith("/"):
            return True
    return False


def parse_listing_card(card: Any, base_url: str) -> Optional[Dict[str, Any]]:
    """Extrae un aviso de una tarjeta de Properati. None si no es parseable."""
    link = None
    for selector in _TITLE_SELECTORS:
        element = card.select_one(selector)
        if element is not None and element.get("href") not in _SKIP_HREFS:
            link = element
            break
    if link is None:
        for anchor in card.find_all("a", href=True):
    ...
    title = text_of(link) or text_of(card.select_one("h2, h3, [class*='title']"))
...
def parse_search_page(html: str, base_url: str) -> List[Dict[str, Any]]:
    ...
    for selector in CARD_SELECTORS:
        found = [c for c in soup.select(selector) if _is_valid_card(c, base_url)]
    ...
    if best_selector:
        ...
        logger.warning(
            "Properati: no se encontraron tarjetas con %s. "
...
def scrape_city(
    ...
    """Scrapea ``pages`` páginas de venta en ``city`` (Properati Ecuador).
    ...
    for page_no in range(1, pages + 1):
        if current_url is None:
            break
        logger.info("Properati [%s] — página %d/%d: %s", city, page_no, pages, current_url)
        ...
        if blocked:
            raise SiteBlockedError(
                f"Properati bloqueó el acceso ({blocked}). "
        ...
        seen_urls |= page_urls
        logger.info("Properati [%s] — página %d: %d avisos (acumulado: %d)", city, page_no, len(page_records), len(records))
        ...
        if page_no < pages:
            ...
            if next_url is None or next_url == current_url:
                # Respaldo determinista (Properati usa ?page=N)
...
def main() -> None:
    """CLI del scraper de Properati Ecuador."""
    parser = argparse.ArgumentParser(
        description="Scraper de Properati Ecuador (properati.com.ec) — Radar de Valorización Urbana. "
    ...
    logger.info("Properati [%s]: %d avisos guardados en %s (%.1f s)",
